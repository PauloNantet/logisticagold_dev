...
export default function OrcamentoHistoryModal({ history, onDelete, onClose, onRestore, onConvertToInvoice }) {
  ...
  const filteredHistory = history.filter(item => 
    ...
    (item.data_emissao || "").toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="modal-body">
          {history.length === 0 ? (
            <div className="empty-clients">
              Nenhum orçamento baixado ainda.
            </div>
          ) : (
            <>
              <div className="table-responsive spreadsheet-container">
                <table className="spreadsheet-table">
                  <thead>
                    <tr>
                      <SortTh label="Nº Orçamento" sortKey="numero" />
                      <SortTh label="Data Emissão" sortKey="data_emissao" />
