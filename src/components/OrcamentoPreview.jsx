...
export default function OrcamentoPreview({ data, total, descontoCalculado, impostoCalculado, finalTotal, isLocked, onBack, onDownload, onBackToHistory }) {
  const formatOrcamentoNumber = (num) => {
    if (!num) return "001";
  ...
  const validadeDias = data.orcamento.validade || "30";
    ...
    <div className="preview-container">
      <div className="preview-actions no-print">
        <button 
          ...
          className="back-btn"
        >
          {isLocked ? "✨ Novo Orçamento" : "← Voltar e Editar"}
        </button>
      ...
      {isLocked && (
        <div className="locked-alert no-print">
          ⚠️ ESTE É UM ORÇAMENTO ARQUIVADO. ALTERAÇÕES NÃO SÃO PERMITIDAS.
        </div>
      )}
      ...
      <div ref={wrapperRef} className="invoice-wrapper">
        ...
        <div className="invoice-header">
          <div className="header-top-row">
            ...
            <h1 className="document-title">ORÇAMENTO</h1>
          </div>
          ...
          <div className="header-bottom-row">
            ...
            <div className="document-info">
              <p><span className="info-label">Número:</span> <span className="info-value">{formatOrcamentoNumber(data.orcamento.numero)}</span></p>
              <p><span className="info-label">Emissão:</span> {formatDateBR(data.orcamento.data)}</p>
              <p className="due-date"><span className="info-label">Validade:</span> {validadeDias}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <div className="summary-section" style={{ width: "50%" }}>
